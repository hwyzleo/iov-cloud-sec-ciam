package net.hwyz.iov.cloud.sec.ciam.service.infrastructure.persistence.converter;

import net.hwyz.iov.cloud.sec.ciam.service.domain.model.DeactivationRequest;
import net.hwyz.iov.cloud.sec.ciam.service.infrastructure.persistence.po.DeactivationRequestPo;
import org.mapstruct.Mapper;
import org.mapstruct.ReportingPolicy;
import org.mapstruct.factory.Mappers;

@Mapper(componentModel = "spring", unmappedTargetPolicy = ReportingPolicy.IGNORE)
public interface DeactivationRequestPoConverter {
    DeactivationRequestPoConverter INSTANCE = Mappers.getMapper(DeactivationRequestPoConverter.class);
    DeactivationRequest toDomain(DeactivationRequestPo po);
    DeactivationRequestPo toPo(DeactivationRequest domain);
}
