...
public class MptAccountController extends BaseController {
    ...
    public ApiResponse<PageResult<UserSearchDocument>> searchUsers(
        ...
        startPage();
        List<UserSearchDocument> list = adminQueryAppService.queryUserList(userId, identityType, identityValue, nickname, registerSource, userStatus, startTime, endTime);
    ...
    public ApiResponse<PageResult<AuditLogSearchDocument>> queryAuditLogs(
            @RequestParam(required = false) String userId,
            @RequestParam(required = false) String eventType,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startTime,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endTime) {
        startPage();
        var result = adminQueryAppService.queryAuditLogs(userId, eventType, startTime, endTime, 0, 0);
        return ApiResponse.ok(getPageResult(result.getItems(), result.getTotal()));
    }
    ...
    public ApiResponse<PageResult<RiskEventSearchDocument>> queryRiskEvents(
            @RequestParam(required = false) String userId,
            @RequestParam(required = false) Integer riskLevel,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startTime,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endTime) {
        startPage();
        var result = adminQueryAppService.queryRiskEvents(userId, riskLevel, startTime, endTime, 0, 0);
        return ApiResponse.ok(getPageResult(result.getItems(), result.getTotal()));
    }
